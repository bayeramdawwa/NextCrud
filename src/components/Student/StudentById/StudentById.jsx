import { getById } from "@/store/actions/students.action";
...
export default function StudentById() {
  ...
  const students = useSelector((state) => state.students.studentById);
  useEffect(() => {
    if (router?.query?.id) {
          ...
          <Card.Body>
            ...
            <div class="d-grid gap-2">
              <Button
                variant="primary"
                onClick={() => {
                  router.push(routes.editstudent(students.id));
                }}
              >
                Edit
              </Button>
